...
int write_out (struct connection *c, const void *_data, int len) {
	...
	while (len) {
		if (c->out_tail->end - c->out_tail->wptr >= len) {
			...
			int y = (int)(c->out_tail->end - c->out_tail->wptr);
			assert (y < len);
...
int read_in (struct connection *c, void *_data, int len) {
	...
	int x = 0;
	while (len) {
		int y = (int)(c->in_head->wptr - c->in_head->rptr);
		if (y > len) {
...
#ifdef _WIN32
#define MAX_CONNECTIONS USHRT_MAX
#else
...
struct connection *create_connection (const char *host, int port, struct session *session, struct connection_methods *methods) {
	...
	WSADATA wsaData;
	assert (WSAStartup (wVersionRequested, &wsaData) == 0);
#endif
	...
	setsockopt (fd, SOL_SOCKET, SO_KEEPALIVE, (char*)&flags, sizeof (flags));
...
#ifdef _WIN32
		if (WSAGetLastError () != WS_E_OPERATION_TIMED_OUT) { continue; }
#else
